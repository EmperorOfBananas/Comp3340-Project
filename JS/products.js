...
function request(url,itemid){
  ...
  xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        	...
        	//adds html
            document.querySelector(`div[itemid="${itemid}"] > .card-main-height`).insertAdjacentHTML("afterbegin",this.responseText)     
      }
  };
...
function alertUser(itemid){
  if(!document.querySelector(`div[product="${itemid}"]`)){//checks if element exists or not
    //adds html
  	document.querySelector(`div[itemid="${itemid}"] > .card-main-height`).insertAdjacentHTML("afterbegin",`<div class="alert alert-warning text-center" product="${itemid}">Must be logged in to add item!</div>`)
  }
}