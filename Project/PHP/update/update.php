...
<div class="form d-flex flex-column align-items-center my-3">
 ...
 <form class="text-center p-3 my-auto border border-2 border-dark rounded-3" action="updateuser.php" method="POST">
   <h3 class="mb-3 text-dark">Update User</h3>
   <?php if($_GET["updated"]){//checks if records were updated
    ...
    if($_SESSION["isadmin"]){//check if admin is logged in
      ...
      $result=queryResultSelect($openConnection,"SELECT UserId, FirstName, LastName FROM UserAdmin EXCEPT SELECT UserId, FirstName, LastName FROM UserAdmin WHERE UserId=1");//retrieve user except of main admin
      echo '<div class="form-floating">
      		  <!--https://getbootstrap.com/docs/5.1/forms/select/-->
              <select onchange="fillFields(this.value)" class="form-select form-select-lg my-1" id="users" name="users" aria-label=".form-select-lg" required>';
      if(mysqli_num_rows($result)>0){//checks if any rows were selected
         echo '<!--options-->
         	   <option hidden>Select User</option>';
       while($row=mysqli_fetch_assoc($result)){
         echo '<option value="'.$row["UserId"].'">'.$row["UserId"].'. '.$row["FirstName"].'</option>';
       }
      }
      echo '	</select>
  ...
  ?>
   ...
   <div class="d-flex flex-column align-items-center">
     <!--link to go back to manage user page-->
