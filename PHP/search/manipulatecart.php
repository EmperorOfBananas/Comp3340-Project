   ...
   $result=queryResultSelect($openConnection, "SELECT Price FROM Products WHERE ItemId=$itemid");//retrieve item price
    ...
    if(isset($_SESSION["inCart"][$id])){//checks if item is in the cart
      ...
      if(isset($_SESSION[$id])){//check if item is part of the session
        if($quantity>$_SESSION[$id]){//checks if manipulated quantity is greater than current cart quantity
           $quantityDiff=$quantity-$_SESSION[$id];//get difference betwen quantities
           $_SESSION["total"]+=$price*$quantityDiff;//calculate total price
        }else if($quantity<$_SESSION[$id]){//checks if manipulated quantity is less than current cart quantity
           $quantityDiff=$_SESSION[$id]-$quantity;//get difference betwen quantities
           $_SESSION["total"]-=$price*$quantityDiff;//calculate total price
        }
