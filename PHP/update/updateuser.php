<?php
	...
	$userid=$_POST["users"];
    $result=queryResultSelect($openConnection,"SELECT Email FROM UserAdmin WHERE Email='{$email}' EXCEPT SELECT Email FROM UserAdmin WHERE UserId=$userid");//retrieve email to check if user with that email already exists
